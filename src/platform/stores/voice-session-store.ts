...
export interface TranscriptEntry {
  ...
  isFinal: boolean;
  isAgent: boolean;
  /** True for messages appended by `tellAgent` (UI bridge). Excluded from voice-intent hooks so they do not re-broadcast to all subscribers. */
  skipVoiceIntent?: boolean;
}
...
interface VoiceSessionState {
  ...
  /** Send a chat message that starts an agent turn without adding to visible transcripts. */
  kickAgentTurn: (message: string) => Promise<void>;
...
export const useVoiceSessionStore = create<VoiceSessionState>((set, get) => ({
  ...
  connect: async () => {
    ...
    try {
      ...
      // Adopt any agent participant that was already in the room when we joined —
      // ParticipantConnected does not fire for pre-existing participants.
      adoptExistingAgentParticipant(room, set);
  ...
  kickAgentTurn: async (message: string) => {
    const { room } = get();
    if (!room?.localParticipant) return;
    const trimmed = message.trim();
    if (!trimmed) return;
    try {
      // Send via lk.chat — this is what the agent framework treats as a user turn.
      // Intentionally NOT added to visible transcripts so it stays invisible to the user.
      await room.localParticipant.sendText(trimmed, { topic: 'lk.chat' });
    } catch (error) {
      console.error('kickAgentTurn error:', error);
    }
  },
...
/**
 * Scans room.remoteParticipants for an already-present agent participant and wires it up.
 * Must be called right after room.connect() resolves in the standard (non-pre-warm) flow,
 * because setupRoomEventListeners runs before connect so the initial scan finds no one,
 * and ParticipantConnected never fires for participants already in the room when we join.
 */
function adoptExistingAgentParticipant(
  room: Room,
...
function registerRpcHandlers(
  ...
  // Serial queue: ensures concurrent callSiteFunction RPCs from the same agent turn
  // execute one-at-a-time. Without this, rapid-fire calls (e.g. industry→role→priority
  // sent in a single agent response) all read stale state before the first one updates
  // sectionsRef, causing later steps to bypass the isWaitingForUserInput gate.
  let siteFnChain: Promise<void> = Promise.resolve();

  // Handler: Call site function
  localParticipant.registerRpcMethod('callSiteFunction', (data) => {
    const entry: Promise<string> = siteFnChain.then(async () => {
      try {
        const payload = JSON.parse(data.payload) as { name: string; args: unknown };
        ...
        // Log suppressed duplicates so they're visible but clearly not a real navigation.
        if (result && typeof result === 'object' && (result as Record<string, unknown>).suppressed_duplicate) {
          console.log('RPC: callSiteFunction THROTTLED (duplicate suppressed)', name, args);
        }
    ...
    // Advance the chain; swallow errors so one failed call doesn't stall the queue.
    siteFnChain = entry.then(() => {}, () => {});
    return entry;
  });
}
